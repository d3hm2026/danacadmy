...
export default function CourseEnrollmentsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleStatus(enrollmentId: string, status: "approved" | "rejected") {
    ...
    await fetch(`/api/admin/courses/${id}/enrollments/${enrollmentId}`, {
      method: "PATCH",
  ...
  async function handleDelete(enrollmentId: string) {
    ...
    setActionId(enrollmentId);
    await fetch(`/api/admin/courses/${id}/enrollments/${enrollmentId}`, { method: "DELETE" });
    await load();
    setActionId(null);
  }
  ...
  return (
    <div dir="rtl">
      ...
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        ...
        <Link href={`/admin/courses/${id}/enrollments`} className="text-sm font-semibold text-[#1a2e5a] border-b-2 border-[#1a2e5a] pb-0.5">طلبات الانتساب</Link>
          ...
          {filtered.map((e) => (
            <div key={e.id} className="bg-white rounded-2xl border border-gray-100 p-5 flex items-center justify-between gap-4">
              ...
              <div className="flex gap-2 flex-shrink-0">
                ...
                <button onClick={() => handleDelete(e.id)} disabled={actionId !== null}
                  className="bg-gray-100 hover:bg-gray-200 text-gray-500 px-3 py-2 rounded-xl text-sm">
                  🗑️
                </button>
              </div>
            </div>
