...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const studentProgress = approvedEnrollments.map((enrollment) => {
    ...
    return { userId, name: enrollment.user.name, phone: enrollment.user.phone, completedLessons, totalLessons, progressPct };
  });
  ...
  const quizStats = course.units
    ...
    .map((unit) => {
      ...
      const avgScore = attempts.length > 0
        ...
        : 0;
      return {
        quizId: quiz.id, quizTitle: quiz.title, unitTitle: unit.title,
        passingScore: quiz.passingScore, totalAttempts: attempts.length,
